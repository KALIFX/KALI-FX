...
# Fetch the latest time from WorldTimeAPI
def fetch_latest_time():
    ...
    try:
        ...
        formatted_time = datetime.strptime(utc_time, "%Y-%m-%dT%H:%M:%S.%f%z").strftime('%Y-%m-%dT%H:%M:%SZ')
        return formatted_time

    except requests.exceptions.RequestException as e:
        print(f"Error fetching data from WorldTimeAPI: {e}")
        return None
...
# New button for updating the last updated time
btn_update_time = tk.Button(frame_buttons, text="Update Last Updated Time", command=handle_update_time)
btn_update_time.grid(row=0, column=3, padx=5)
